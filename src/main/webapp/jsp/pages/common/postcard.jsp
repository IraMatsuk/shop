        ...
        <div class="row" style="background: whitesmoke">
            ...
            <div class="col-auto">
                <form name="sortByPrice" action="${absolutePath}/controller">
                    ...
                    <button type="submit" class="btn btn-warning btn-sm"><fmt:message key="postcard.sort_by_price"/></button>
                </form>
            </div>
            <div class="col-auto">
                <form name="sortByPopularity" action="${absolutePath}/controller">
                    ...
                    <button type="submit" class="btn btn-danger btn-sm"><fmt:message key="postcard.sort_by_popularity"/></button>
                </form>
            </div>
        </div>
    </div>
    <div class="box box_padding catalog-wrap catalog-body">
        <div class="catalog">
            <c:choose>
                <c:when test="${empty postcard_list}">
                    <h3 class="text-center"><fmt:message key="postcard.empty"/> </h3>
                </c:when>
                <c:otherwise>
                    <c:forEach items="${postcard_list}" var="postcard">
                        <div class="catalog-item">
                            <div class="product">
                                ...
                                <div class="product_figure">
                                    <c:choose>
                                        <c:when test="${postcard.picturePath eq 'picture/default-image.png'}">
                                ...
                                <div class="">
                                    <div class="product_price "><b id="price"><c:out value="${postcard.price}"/> </b> <fmt:message key="postcard.product_money"/> </div>
                                    <div class="product_price" ><b id="discount"><fmt:message key="postcard.product_discount"/></b> <fmt:formatNumber type="number"  maxFractionDigits="0" value="${postcard.discount * 100}"/>%  </div>
                                    <div class="product_price"><b id="total_price"><fmt:message key="postcard.product_price"/></b> <fmt:formatNumber type="number" minFractionDigits="2" maxFractionDigits="2"  value="${postcard.price - postcard.discount * postcard.price}"/>   </div>
                                    ...
                                    <c:if test="${user.role eq 'ADMIN'}">
                                        <form name="UploadPhoto" method="post" action="${absolutePath}/controller" enctype="multipart/form-data">
                                            ...
                                            <div class="form-group mb-2">
                                                <label class="form-label"><fmt:message key="postcard.picture"/></label>
                                                <input type="file" name="picture_path" class="form-control form-control-sm">
                                            </div>
                                            <button type="submit" class="btn btn-primary btn-sm mb-2"><fmt:message key="postcard.insert_postcard"/></button>
                                        </form>
                                        <div>
                                            <div class="row">
                                                <div class="col">
                                                    <a class="btn btn-info btn-sm" href="${absolutePath}/controller?command=go_to_update_product_page&id=<c:out value="${postcard.postcardId}"/>" role="button"><fmt:message key="profile.update"/></a>
                                                </div>
                                                <div class="col">
                                                    <form action="${absolutePath}/controller" method="post">
